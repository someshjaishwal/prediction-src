...
class SpectrogramParser(AudioParser):
    ...
    def parse_audio(self, audio_path):
        ...
        n_fft = int(self.sample_rate * self.window_size) ## default : 16000 * 0.02 = 320
        win_length = n_fft
        hop_length = int(self.sample_rate * self.window_stride) ## default : 16000 * 0.01 = 160
        # STFT
...
class SpectrogramDataset(Dataset, SpectrogramParser):
    ...
    def parse_transcript(self, transcript_path):
        ...
        # print(f'transcript embedd : {transcript}')
        return transcript
...
class SpectrogramDataset2(Dataset, SpectrogramParser):
    def __init__(self, audio_conf, manifest_filepath, labels, normalize=False, augment=False):
        """
        Dataset that loads tensors via a csv containing file paths to audio files and transcripts separated by
        a comma. Each new line is a different sample. Example below:
        ...
        self.size = len(ids)
        self.labels_map = dict([(labels[i], i) for i in range(len(labels))])
        super(SpectrogramDataset2, self).__init__(audio_conf, normalize, augment)
